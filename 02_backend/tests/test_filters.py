...
def _search(query, settings, top_k=17, **filters):
...
class TestFachbereichFilter:
    ...
    def test_wd6_filter_returns_only_wd6(self, require_qdrant, settings):
        """We have 2 WD 6 docs: WD 6-052-24 and WD 6-094-23."""
        results = _search(BROAD_QUERY, settings, fachbereich="WD 6")
    ...
    def test_wd6_filter_finds_both_docs(self, require_qdrant, settings):
        results = _search(BROAD_QUERY, settings, fachbereich="WD 6")
    ...
    def test_wd3_filter_returns_only_wd3(self, require_qdrant, settings):
        results = _search(BROAD_QUERY, settings, fachbereich="WD 3")
        assert len(results) > 0
        for r in results:
            fb_prefix = r.aktenzeichen.split(" - ")[0].strip()
            assert fb_prefix == "WD 3"

    def test_eu6_filter_returns_only_eu6(self, require_qdrant, settings):
        results = _search(BROAD_QUERY, settings, fachbereich="EU 6")
        assert len(results) > 0
        for r in results:
            fb_prefix = r.aktenzeichen.split(" - ")[0].strip()
...
class TestDocumentTypeFilter:
    ...
    def test_ausarbeitung_filter(self, require_qdrant, settings):
        results = _search(BROAD_QUERY, settings, document_type="Ausarbeitung")
    ...
    def test_sachstand_filter(self, require_qdrant, settings):
        results = _search(BROAD_QUERY, settings, document_type="Sachstand")
        for r in results:
            assert r.document_type == "Sachstand", (
                f"Filter was Sachstand but got '{r.document_type}' for {r.aktenzeichen}"
            )

    def test_kurzinformation_filter(self, require_qdrant, settings):
        results = _search(BROAD_QUERY, settings, document_type="Kurzinformation")
        for r in results:
...
class TestDateRangeFilter:
    ...
    def test_single_year_filter(self, require_qdrant, settings):
        """Filter to 2023 — all results must have completion_date in 2023."""
        results = _search(BROAD_QUERY, settings, date_from="2023", date_to="2023")
        for r in results:
    ...
    def test_year_range_filter(self, require_qdrant, settings):
        """Filter 2022-2023 — all results must have year in that range."""
        results = _search(BROAD_QUERY, settings, date_from="2022", date_to="2023")
    ...
    def test_only_from_date(self, require_qdrant, settings):
        """date_from=2024 with no date_to — all results from 2024 onwards."""
        results = _search(BROAD_QUERY, settings, date_from="2024")
        for r in results:
            parsed = date.fromisoformat(r.completion_date)
    ...
    def test_only_to_date(self, require_qdrant, settings):
        """date_to=2022 with no date_from — all results from 2022 or earlier."""
        results = _search(BROAD_QUERY, settings, date_to="2022")
        for r in results:
            parsed = date.fromisoformat(r.completion_date)
...
class TestCombinedFilters:
    ...
    def test_fachbereich_plus_date_narrows_results(self, require_qdrant, settings):
        """WD 6 + year 2023 should return only WD 6-094-23 (not WD 6-052-24)."""
        results = _search(
            BROAD_QUERY, settings,
            fachbereich="WD 6",
            date_from="2023",
            date_to="2023",
        )
    ...
    def test_impossible_filter_returns_empty(self, require_qdrant, settings):
    ...
    def test_combined_all_three(self, require_qdrant, settings):
...
class TestNoFilter:
    def test_unfiltered_returns_many_results(self, require_qdrant, settings):
        """Without filters and a broad query, we should get most of the 17 documents."""
        results = _search(BROAD_QUERY, settings, top_k=17)
